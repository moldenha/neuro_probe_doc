...
class NameColorDialog:
    def __init__(self, parent, default_name = "", default_color = "#ffffff"):
        ...
        ttk.Button(button_frame, text="Proceed", command=self.save).pack(side="left", padx=5)
...
# This is a specific version for the edit button, that allows the user to then choose to change the location
# Of the probe or not
class NameColorDialog_edit:
    def __init__(self, parent, default_name = "", default_color = "#ffffff"):
        self.result = None
        ...
        ttk.Button(button_frame, text="Cancel", command=self.cancel).pack(side="left", padx=5)
        ttk.Button(button_frame, text="Change Location", command=self.edit).pack(side="left", padx=5)
    ...
    def save(self):
        name = self.date_var.get().strip()
        if name:
            self.result = (name, self.color, False)
        self.window.destroy()

    def edit(self):
        name = self.date_var.get().strip()
        if name:
            self.result = (name, self.color, True)
        self.window.destroy()
...
def get_name_and_color_edit(parent, name = '', color = '#ffffff'):
    dialog = NameColorDialog_edit(parent, default_name = name, default_color = color)
    return dialog.result
